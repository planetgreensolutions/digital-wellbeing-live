                  ...
                  <table class="table table-striped table-bordered" style="min-width:1500px">
                     <thead>
                        <tr>		
                           ...
                           <th scope="col" width="10%">{{ lang('shortlisted_by') }}</th>
							...
							<tr >
								...
								<td>
									<div class="catFormWrapper">
										<span class="badge category_{{ $submission->formManager->fm_form_category_id }}">{{ ucWords($submission->formManager->getData('fm_category_en')) }}</span>
										<span class="badge category_{{ $submission->formManager->fm_form_category_id }}">{{ ucWords($submission->formManager->getData('fm_category')) }}</span>
								...
								<td>	
									<div class="catFormWrapperAR">			
										<span class="badge2 category_{{ $submission->formManager->fm_form_category_id }} formType_{{ $submission->formManager->fm_form_type_id }}">{{ ucWords($submission->formManager->getData('fm_sub_category_en')) }}</span>
										<span class="badge2  category_{{ $submission->formManager->fm_form_category_id }} formType_{{ $submission->formManager->fm_form_type_id }}">{{ ucWords( $submission->formManager->getData('fm_sub_category')) }}</span>
									</div>
								</td>								
								<td>
								...
								<td>
									<div>{{ $submission->formOwner->name }}
										...
										<div class="flagWrapper">
											...
											<span>{{ $submission->formOwner->userNationality->getData('country_name') }}</span>
										</div>
									</div>
								</td>
								<td style="text-align:right">{{ (isset($submission->shortLister))?$submission->shortLister->name:'-' }}</td>
			...
			sendAjax($(this).attr('href'),'GET',{},function(data){
				...
				if(data.status){
					if(data.formStatus == 1){
						...
						$_elem.removeClass('btn-outline-danger').addClass('btn-outline-success').html('{{ lang("add_to_shortlist") }}');
					}
					swal.fire({title:data.message,type:'success'});
