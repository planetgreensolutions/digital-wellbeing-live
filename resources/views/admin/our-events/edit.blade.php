				...
				<div class="card">
					...
					<?php /* <section class="basic_settings">
							...
							<section class="basic_settings">
								<div class="row"> 	
									<div class="col-sm-6 form-group">
									{!! getSinglePlUploadControl('Upload Banner Image 500x500 (Max 2 MB) (jpg,jpeg) ','events_image',['jpg','jpeg'],'image','Select File',null,null,@$postDetails->getData('events_image'),$postType) !!}
									...
									<select class="form-control" id="news_category_id" name="post[category_id]">
										@foreach($EventsCategoryList as $nclist)
											<option  value="{{ $nclist->category_id }}" <?php echo ( $postDetails->getData('post_category_id') == $nclist->category_id )?'selected =="selected"':""; ?>>{{ $nclist->category_title }}</option>
										@endforeach
									</select> 
								</div>
								</div>
								...
								<div class="row"> 	
									<div class="col-sm-6 form-group">
										...
										<input type="text" name="meta[date][publish_date]" id="publish_date" class="form-control datepicker" placeholder="" value="{{ ($postDetails->getData('publish_date'))?date('m/d/Y',strtotime($postDetails->getData('publish_date'))):'' }}"  />
									</div>	
