	...
	<div class="row">
		<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
			<div class="page-header">
				<h2 class="pageheader-title">Edit Council Members
				<?php /*
						...
						<section class="basic_settings">
							<div class="row"> 	
								...
								<div class="col-sm-6 form-group">
									...
									<input type="text" name="post[title_arabic]" id="post_title_arabic" class="form-control" placeholder="" value="{{ $postDetails->post_title_arabic }}"  />
								</div>	
							</div>
							...
							<div class="row"> 	
								<div class="col-sm-6 form-group">
									...
									<input type="text" name="meta[text][subtitle]" id="subtitle" class="form-control" placeholder="" value="{{ $postDetails->getMeta('subtitle') }}"  />
								</div>	
								<div class="col-sm-6 form-group">
									<label>Designation [Arabic]</label>
									<input type="text" name="meta[text][subtitle_arabic]" id="subtitle_arabic" class="form-control" placeholder="" value="{{ $postDetails->getMeta('subtitle_arabic') }}"  />
								</div>	
							</div>

							<div class="col-sm-6 form-group">
								<label for="council_role" class="col-form-label">Role <em>*</em></label>
								<select class="form-control" id="council_role" name="meta[text][council_role]" required>
									<option value="">Select Role</option>
									@foreach($council_roles as $role)
									<option <?php echo ( $role->post_id == $postDetails->getData('council_role'))?'selected =="selected"':""; ?> value="{{$role->getData('post_id')}}">{{$role->getData('post_title')}}</option>
									@endforeach
								</select>
							</div>
							...
							<div class="row"> 	
								<div class="col-sm-6 form-group">
									...
									<textarea name="meta[text][description]" class="form-control ckeditorEn" id="description" placeholder="" >{{$postDetails->getMeta('description') }}</textarea> 
								</div>
								<div class="col-sm-6 form-group">
									<label>Description [Arabic]</label>
									<textarea name="meta[text][description_arabic]" class="form-control ckeditorAr" id="description_arabic" placeholder="" >{{$postDetails->getMeta('description_arabic') }}</textarea> 
								</div>
							</div>
							...
							<div class="row"> 	
								<div class="col-sm-6 form-group">
									...
									{!! getSinglePlUploadControl('Upload Image (Max 2 MB) (jpg,jpeg) 430*474 ','council_image',['jpg','jpeg'],'image','Select File',null,null,$postDetails->getData('council_image'),$postType) !!}
								</div>
							</div>
						</section>
