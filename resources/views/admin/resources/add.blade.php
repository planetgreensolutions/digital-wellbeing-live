							...
							<div class="row"> 	
							   <div class="col-sm-6 form-group">
								...
								<select class="form-control" id="resource_lang" name="meta[text][resource_lang]">
									<option <?php echo (  old('resource_lang')  == 'en' )?'selected =="selected"':""; ?> value="en">English</option>
									<option <?php echo ( old('resource_lang') == 'ar' )?'selected =="selected"':''; ?> value="ar">Arabic</option>
								</select>
										...
										<div class="col-sm-6 form-group">
											...
											<select class="form-control" id="resource_category_id" name="post[category_id]">
												@foreach($resourceSubCategoryList as $rcSlist)
													<option  value="{{ $rcSlist->category_id }}">{{ $rcSlist->category_title }}</option>
												@endforeach
											</select>
										</div>
									...
									<div class="col-sm-6 form-group">
										...
										<select class="form-control" id="resource_sub_category_id" name="post[sub_category_id]">
												@foreach($resourceCategoryList as $rclist)
													<option  value="{{ $rclist->category_id }}">{{ $rclist->category_title }}</option>
												@endforeach
										</select>
									</div>
								</div>
							...
							<section class="basic_settings">
								<div class="row"> 	
									<div class="col-sm-6 form-group">
										...
										{!! getSinglePlUploadControl('Upload  Image (Max 2 MB) (jpg,jpeg) 500*500 ','resources_image',['jpg','jpeg'],'image','Select File',null,null,@Input::old('meta')['text']['resources_image'],$postType) !!}
									</div>
								</div>
								<div class="row"> 	
									<div class="col-sm-6 form-group">
										...
										<input type="hidden" name="post[title_arabic]" id="post_title_arabic" dir="rtl" class="form-control" placeholder="" value="{{ old('post_title') }}"  required/>
								...
								<div class="row"> 	
									<div class="col-sm-6 form-group">
										...
										<input type="text" name="meta[text][android_link]" id="android_link" class="form-control" placeholder="" value="{{ old('meta')['android_link'] }}"  />
									</div>	
									</div>
								<div class="row"> 										
									<div class="col-sm-6 form-group">
										<label>Iphone Link</label>
										<input type="text" name="meta[text][iphone_link]" id="iphone_link" class="form-control" placeholder="" value="{{ old('meta')['iphone_link'] }}"  />
									</div>	
								</div>
								...
								<div class="row"> 	
									<div class="col-sm-6 form-group">
										...
										<input type="url" name="meta[text][website_link]" id="website_link" class="form-control" placeholder="" value="{{ old('meta')['website_link'] }}"  />
									</div>	
									<div class="col-sm-6 form-group">
									{!! getSinglePlUploadControl('Upload Attachment  (Max 2 MB) (doc,docx,pdf) ','resources_file',['doc','docx','pdf'],'file','Select File',null,null,@old('meta')['resources_file'] ,$postType) !!}
