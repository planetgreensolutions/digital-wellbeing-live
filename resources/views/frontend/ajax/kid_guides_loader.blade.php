@if(!empty($guides) && $guides->isNotEmpty())
    @foreach($guides as $item)
        @php
            ...
            if($item->getData('kid_guides_image')){
            $guideImage=PP($item->getData('kid_guides_image'));
            }
        @endphp
        <div class="item_">
            <div class="img_box">
                <div class="img_ b-lazy" data-src="{{$guideImage}}"></div>
                @if(!empty($item->getData('kid_guides_pdf')))
                <a data-fancybox data-type="iframe" href="{{PP($item->getData('kid_guides_pdf')) }}">
                <div class="dwnload_">
                <div class="icon_">
